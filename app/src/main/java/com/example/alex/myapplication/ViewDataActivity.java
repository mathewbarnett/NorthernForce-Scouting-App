...
//       data = Arrays.copyOf(data, fin);
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            for (int l = 0; l < data[i].length; l++) {
                ...
                if (data[i][l].contains("Hiigh")) {
                    percents[percentIndex].setNum1(percents[percentIndex].getNum1() + 1);
                    percents[percentIndex].setNum2(l);
                    percentIndex++;
                }
     ...
     //   Log.v("DataBase", "YO SUP");
        ...
        for (int g = 0; g < percents.length; g++) {
            ...
            Log.v("DataBase", "BASE ABOUT TO BE COMPUTED: " + percents[g].getNum1());
            percents[g].setNum1((int) ((((double) percents[g].getNum1()) / divisor) * 100));

            Log.v("DataBase", "PERCENT: " + percents[g].getNum1());
            Log.v("DataBase", "\n");
        ...
        for (int g = 0; g < percents.length; g++) {
            ...
            if (percents[g].getNum1() > 0) {
                view.setText(view.getText() + dirt[percents[g].getNum2()] + ": " + percents[g].getNum1() + "%\n");
            }
