package com.example.alex.myapplication.TheBlueAlliance;


import android.util.Log;

import com.example.alex.myapplication.TheBlueAlliance.event.Event;
import com.example.alex.myapplication.TheBlueAlliance.event.SimpleEvent;
import com.example.alex.myapplication.TheBlueAlliance.matches.Match;
import com.example.alex.myapplication.TheBlueAlliance.teams.Team;
import com.example.alex.myapplication.TheBlueAlliance.util.Constants;
...
public class BlueAlliance {
     ...
     * @throws IOException Throws IOException on connection/reader errors
     */
    public ArrayList<SimpleEvent> getEventList(String year) throws IOException {
        @SuppressWarnings("unchecked")
              String URL =   createURL(Constants.BLUE_ALLIANCE_EVENT_LIST + year);
        Log.v("blueAlliance", "URL is " + URL);
        Log.v("blueAlliance", "is returned empty? " + deserializeJSONArray(new TypeToken<ArrayList<SimpleEvent>>() {
                }.getType(),
                createReader(URL)).isEmpty());
        ArrayList<SimpleEvent> list = (ArrayList<SimpleEvent>) (List<?>) deserializeJSONArray(new TypeToken<ArrayList<SimpleEvent>>() {
        }.getType(),
                createReader(URL));
        if(list == null){
            Log.v("blueAlliance", "list is null");
        }
        return list;
    }
     ...
     */
    private ArrayList<BlueAllianceObject> deserializeJSONArray(Type type, Reader reader) {
        Gson gson = new Gson();
    ...
    private Reader createReader(String url) throws IOException {
        ...
        urlConnection.addRequestProperty("X-TBA-App-Id", "frc172:java-blue-alliance-api:v1.0.2");
    ...
    private String createURL(String append) {
        return Constants.BLUE_ALLIANCE_BASE_URL_V1 + append;
    }
